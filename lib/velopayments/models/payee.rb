  ...
  class Payee
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VeloPayments::Payee` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VeloPayments::Payee`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [payee_id, payor_refs, email, address, country, display_name, payment_channel, challenge, language, accept_terms_and_conditions_timestamp, cellphone_number, payee_type, company, individual, created, grace_period_end_date, last_ofac_check_timestamp, marketing_opt_ins, ofac_status, onboarded_status, social].hash
    end
