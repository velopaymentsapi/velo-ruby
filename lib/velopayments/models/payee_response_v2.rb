...
module VeloPayments
  class PayeeResponseV2
    ...
    attr_accessor :ofac_override

    attr_accessor :ofac_override_reason

    attr_accessor :ofac_override_timestamp
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VeloPayments::PayeeResponseV2` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VeloPayments::PayeeResponseV2`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'ofac_override')
        self.ofac_override = attributes[:'ofac_override']
      end

      if attributes.key?(:'ofac_override_reason')
        self.ofac_override_reason = attributes[:'ofac_override_reason']
      end

      if attributes.key?(:'ofac_override_timestamp')
        self.ofac_override_timestamp = attributes[:'ofac_override_timestamp']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          ofac_override == o.ofac_override &&
          ofac_override_reason == o.ofac_override_reason &&
          ofac_override_timestamp == o.ofac_override_timestamp &&
    ...
    def hash
      [payee_id, payor_refs, email, onboarded_status, ofac_status, language, created, country, display_name, payee_type, disabled, disabled_comment, disabled_updated_timestamp, address, individual, company, cellphone_number, last_ofac_check_timestamp, ofac_override, ofac_override_reason, ofac_override_timestamp, grace_period_end_date, enhanced_kyc_completed, kyc_completed_timestamp, pause_payment, pause_payment_timestamp, marketing_opt_in_decision, marketing_opt_in_timestamp, accept_terms_and_conditions_timestamp].hash
    end
