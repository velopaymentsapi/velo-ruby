  ...
  class RejectedPayment
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VeloPayments::RejectedPayment` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VeloPayments::RejectedPayment`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      reason_validator = EnumAttributeValidator.new('String', ["UNKNOWN_REMOTE_ID", "UNKNOWN_SOURCE_ACCOUNT", "UNSUPPORTED_CURRENCY", "PAYMENT_VALUE_TOO_HIGH", "UNSUPPORTED_COUNTRY", "UNSUPPORTED_CURRENCY_FOR_COUNTRY", "PAYEE_NOT_ONBOARDED", "COMPLIANCE_ISSUE", "DUPLICATE_PAYOR_PAYMENT_ID", "CURRENCY_MISMATCH", "DECLINED_INVITE", "PENDING_INVITE", "BADLY_FORMED_DATA_ROW", "OTHER"])
    ...
    def reason=(reason)
      validator = EnumAttributeValidator.new('String', ["UNKNOWN_REMOTE_ID", "UNKNOWN_SOURCE_ACCOUNT", "UNSUPPORTED_CURRENCY", "PAYMENT_VALUE_TOO_HIGH", "UNSUPPORTED_COUNTRY", "UNSUPPORTED_CURRENCY_FOR_COUNTRY", "PAYEE_NOT_ONBOARDED", "COMPLIANCE_ISSUE", "DUPLICATE_PAYOR_PAYMENT_ID", "CURRENCY_MISMATCH", "DECLINED_INVITE", "PENDING_INVITE", "BADLY_FORMED_DATA_ROW", "OTHER"])
    ...
    def hash
      [remote_id, currency_type, amount, source_account_name, payor_payment_id, reason, line_number, message].hash
    end
