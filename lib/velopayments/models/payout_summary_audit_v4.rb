  ...
  class PayoutSummaryAuditV4
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VeloPayments::PayoutSummaryAuditV4` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VeloPayments::PayoutSummaryAuditV4`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [payout_id, payor_id, status, date_time, submitted_date_time, instructed_date_time, withdrawn_date_time, total_payments, total_incomplete_payments, total_returned_payments, source_account_summary, fx_summaries, payout_memo, payout_type].hash
    end
