  ...
  class PayeeDeltaV4
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VeloPayments::PayeeDeltaV4` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VeloPayments::PayeeDeltaV4`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@payee_country.nil? && @payee_country.to_s.length > 2
        invalid_properties.push('invalid value for "payee_country", the character length must be smaller than or equal to 2.')
      end

      if !@payee_country.nil? && @payee_country.to_s.length < 2
        invalid_properties.push('invalid value for "payee_country", the character length must be great than or equal to 2.')
      end

      pattern = Regexp.new(/^[A-Z]{2}$/)
      if !@payee_country.nil? && @payee_country !~ pattern
        invalid_properties.push("invalid value for \"payee_country\", must conform to the pattern #{pattern}.")
      end
    ...
    def valid?
      ...
      return false if !@payee_country.nil? && @payee_country.to_s.length > 2
      return false if !@payee_country.nil? && @payee_country.to_s.length < 2
      return false if !@payee_country.nil? && @payee_country !~ Regexp.new(/^[A-Z]{2}$/)
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] payee_country Value to be assigned
    def payee_country=(payee_country)
      if payee_country.nil?
        fail ArgumentError, 'payee_country cannot be nil'
      end

      if payee_country.to_s.length > 2
        fail ArgumentError, 'invalid value for "payee_country", the character length must be smaller than or equal to 2.'
      end

      if payee_country.to_s.length < 2
        fail ArgumentError, 'invalid value for "payee_country", the character length must be great than or equal to 2.'
      end

      pattern = Regexp.new(/^[A-Z]{2}$/)
      if payee_country !~ pattern
        fail ArgumentError, "invalid value for \"payee_country\", must conform to the pattern #{pattern}."
      end
