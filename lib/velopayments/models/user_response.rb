...
module VeloPayments
  class UserResponse
    ...
    def list_invalid_properties
      ...
      if !@company_name.nil? && @company_name.to_s.length > 128
        invalid_properties.push('invalid value for "company_name", the character length must be smaller than or equal to 128.')
      end
    ...
    def valid?
      ...
      return false if !@company_name.nil? && @company_name.to_s.length > 128
      ...
      mfa_status_validator = EnumAttributeValidator.new('String', ["REGISTERED", "UNREGISTERED", "VERIFIED"])
    ...
    def secondary_contact_number=(secondary_contact_number)
      if secondary_contact_number.nil?
        fail ArgumentError, 'secondary_contact_number cannot be nil'
      end

      pattern = Regexp.new(/^\+[1-9]\d{1,14}$/)
      if secondary_contact_number !~ pattern
    ...
    def company_name=(company_name)
      ...
      if company_name.to_s.length > 128
        fail ArgumentError, 'invalid value for "company_name", the character length must be smaller than or equal to 128.'
      end
    ...
    def mfa_status=(mfa_status)
      validator = EnumAttributeValidator.new('String', ["REGISTERED", "UNREGISTERED", "VERIFIED"])
    ...
    def hash
      [id, status, email, sms_number, primary_contact_number, secondary_contact_number, first_name, last_name, entity_id, company_name, roles, user_type, mfa_type, mfa_status, locked_out, locked_out_timestamp].hash
    end
