  ...
  class CreateFundingAccountRequestV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VeloPayments::CreateFundingAccountRequestV2` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VeloPayments::CreateFundingAccountRequestV2`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@account_name.nil? && @account_name.to_s.length > 22
      ...
      if !@account_number.nil? && @account_number.to_s.length < 4
      ...
      if !@routing_number.nil? && @routing_number.to_s.length < 6
        invalid_properties.push('invalid value for "routing_number", the character length must be great than or equal to 6.')
      end
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["FBO", "PRIVATE"])
      return false unless type_validator.valid?(@type)
      return false if @name.nil?
      return false if @name.to_s.length > 30
      return false if @name.to_s.length < 3
      return false if @payor_id.nil?
      return false if !@account_name.nil? && @account_name.to_s.length > 22
      ...
      return false if !@account_number.nil? && @account_number.to_s.length < 4
      return false if !@routing_number.nil? && @routing_number.to_s.length > 9
      return false if !@routing_number.nil? && @routing_number.to_s.length < 6
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["FBO", "PRIVATE"])
    ...
    def routing_number=(routing_number)
      ...
      if routing_number.to_s.length < 6
        fail ArgumentError, 'invalid value for "routing_number", the character length must be great than or equal to 6.'
      end
    ...
    def hash
      [type, name, payor_id, account_name, account_number, routing_number, country_code].hash
    end
