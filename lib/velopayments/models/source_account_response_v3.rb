...
module VeloPayments
  class SourceAccountResponseV3
    ...
    attr_accessor :pooled

    attr_accessor :customer_id

    # The physical account id (will not be set for DECOUPLED accounts).
    attr_accessor :physical_account_id

    attr_accessor :notifications

    attr_accessor :auto_top_up_config
    ...
    def self.openapi_types
      {
        ...
        :'deleted_at' => :'Time',
        :'transmission_types' => :'Array<String>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VeloPayments::SourceAccountResponseV3` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VeloPayments::SourceAccountResponseV3`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'transmission_types')
        if (value = attributes[:'transmission_types']).is_a?(Array)
          self.transmission_types = value
        end
      end
    end
    ...
    def hash
      [id, balance, currency, funding_ref, physical_account_name, rails_id, payor_id, name, pooled, customer_id, physical_account_id, notifications, auto_top_up_config, type, country, deleted, user_deleted, deleted_at, transmission_types].hash
    end
