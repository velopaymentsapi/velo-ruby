  ...
  class SupportedCountry2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VeloPayments::SupportedCountry2` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VeloPayments::SupportedCountry2`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      iso_country_code_validator = EnumAttributeValidator.new('String', ["US", "GB", "FR", "CA", "JP"])
    ...
    def iso_country_code=(iso_country_code)
      validator = EnumAttributeValidator.new('String', ["US", "GB", "FR", "CA", "JP"])
