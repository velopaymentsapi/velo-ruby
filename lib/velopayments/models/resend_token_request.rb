  ...
  class ResendTokenRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VeloPayments::ResendTokenRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VeloPayments::ResendTokenRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      token_type_validator = EnumAttributeValidator.new('String', ["INVITE_MFA_USER", "MFA_REGISTRATION"])
    ...
    def token_type=(token_type)
      validator = EnumAttributeValidator.new('String', ["INVITE_MFA_USER", "MFA_REGISTRATION"])
    ...
    def hash
      [token_type, verification_code].hash
    end
