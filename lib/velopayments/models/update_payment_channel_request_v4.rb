...
module VeloPayments
  class UpdatePaymentChannelRequestV4
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VeloPayments::UpdatePaymentChannelRequestV4` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VeloPayments::UpdatePaymentChannelRequestV4`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@payment_channel_name.nil? && @payment_channel_name.to_s.length > 30
        invalid_properties.push('invalid value for "payment_channel_name", the character length must be smaller than or equal to 30.')
      end

      if !@payment_channel_name.nil? && @payment_channel_name.to_s.length < 1
        invalid_properties.push('invalid value for "payment_channel_name", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@payment_channel_name.nil? && @payment_channel_name.to_s.length > 30
      return false if !@payment_channel_name.nil? && @payment_channel_name.to_s.length < 1
    ...
    # Custom attribute writer method with validation
    # @param [Object] payment_channel_name Value to be assigned
    def payment_channel_name=(payment_channel_name)
      if payment_channel_name.nil?
        fail ArgumentError, 'payment_channel_name cannot be nil'
      end

      if payment_channel_name.to_s.length > 30
        fail ArgumentError, 'invalid value for "payment_channel_name", the character length must be smaller than or equal to 30.'
      end

      if payment_channel_name.to_s.length < 1
        fail ArgumentError, 'invalid value for "payment_channel_name", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [routing_number, account_number, iban, payment_channel_name, country_code, account_name, currency].hash
    end
