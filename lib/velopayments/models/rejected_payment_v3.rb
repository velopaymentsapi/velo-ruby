  ...
  class RejectedPaymentV3
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VeloPayments::RejectedPaymentV3` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VeloPayments::RejectedPaymentV3`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @payor_payment_id.to_s.length > 40
      return false if @payor_payment_id.to_s.length < 0
    ...
    def hash
      [remote_id, currency_type, amount, source_account_name, payor_payment_id, remote_system_id, payment_metadata, reason, reason_code, line_number, message].hash
    end
