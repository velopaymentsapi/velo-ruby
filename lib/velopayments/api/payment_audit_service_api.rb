...
module VeloPayments
  class PaymentAuditServiceApi
    ...
    def get_payments_for_payout_with_http_info(payout_id, opts = {})
      ...
      if @api_client.config.client_side_validation && !opts[:'page_size'].nil? && opts[:'page_size'] > 100
        fail ArgumentError, 'invalid value for "opts[:"page_size"]" when calling PaymentAuditServiceApi.get_payments_for_payout, must be smaller than or equal to 100.'
      end

      if @api_client.config.client_side_validation && !opts[:'page_size'].nil? && opts[:'page_size'] < 1
        fail ArgumentError, 'invalid value for "opts[:"page_size"]" when calling PaymentAuditServiceApi.get_payments_for_payout, must be greater than or equal to 1.'
      end
    ...
    # @option opts [Integer] :page Page number. Default is 1.
    # @option opts [Integer] :page_size The number of results to return in a page
    # @return [Array<(PaymentDeltaResponseV4, Integer, Hash)>] PaymentDeltaResponseV4 data, response status code and response headers
    def list_payment_changes_v4_with_http_info(payor_id, updated_since, opts = {})
      ...
      # return_type
      return_type = opts[:return_type] || 'PaymentDeltaResponseV4' 
    ...
    def list_payments_audit_with_http_info(opts = {})
      ...
      # return_type
      return_type = opts[:return_type] || 'ListPaymentsResponseV3' 
